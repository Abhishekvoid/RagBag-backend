...
class DocumentListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if chapter_id:
            ...
            logger.info(
                f"Saved document {document.id} for user {self.request.user.id} "
                f"{'linked to chapter ' + chapter_id if chapter else '(no chapter)'}"
            )

            if chapter:
                process_document_for_existing_chapter.delay(str(document.id), str(chapter.id))
                logger.info(f"Triggered processing for existing chapter {chapter.id}")
            else:
                create_chapter_from_document.delay(str(document.id))
                logger.info(f"Triggered smart chapter creation for new document {document.id}")

                
# class ChapterDocumentUploadView(APIView):
...
#         logger.info(f"Successfully saved document record {document.id} to the database. Chapter: {chapter_id}")
#         logger.info(f"File URL generated by storage backend: {document.file.url}")

#         if chapter is None:

#             create_chapter_from_documen
